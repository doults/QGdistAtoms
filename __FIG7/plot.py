...
# disorder_dir = current_dir + 'Data_N5/disorder/B40_Omega8_Delta16/'
# fid_dis      = np.load(disorder_dir + 'variance0.01/fid.npy')
# fid_dis_ave    = np.average(fid_dis, axis=1)
# fid_dis_errors = np.std(fid_dis, axis=1)
...
# load_dir = current_dir + '/DATA_N5/randomized/B40_Omega8_Delta16/variance0.0'
...
#
#
# decay_errors = np.exp(-Gamma * 1/4 * (decay_00 + 2*decay_01 + decay_11))
...
#
# inset.scatter(areas/2, 1-fid_dis_ave + 1 - decay_errors, s=0.4, lw=0.6, marker='^', color='darkred', alpha=1, zorder=3)
# inset.scatter(areas/2, 1-fids + 1 - decay_errors, s=0.4, lw=0.6, color='darkblue', alpha=1, zorder=3)
# inset.set_ylim(6e-4, 2e-1)
#
#
# ax1.fill_between(areas/2, 1-fid_dis_ave, 1-fid_dis_ave+fid_dis_errors, color='darkred', alpha=0.2)
