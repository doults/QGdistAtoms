...
Tlist = list(np.linspace(1., 0.8, 41)[:20])
for ii, T in enumerate(Tlist):
    print(f'-------------------- {ii} : {T} : {round(T * Omega_0 * 0.7527, 4)} --------------------')
    area = np.round(T*Omega_0*0.7527, decimals=4)
    in_guess = np.load(f'Data_N{N}/improved/B40_Omega8_Delta16/Area{area}/Opt_par.npy')
    for syst in systems:
    ...
    optimizer.configure(systems      =systems,
                        ...
                        rk_pre       =1e-10,
                        factors      =[1, 2, 1])
    optimizer.BFGS(maxiter=40, ftol=1e-9, disp=False)
