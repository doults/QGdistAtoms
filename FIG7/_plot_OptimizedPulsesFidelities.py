...
two_theta   = np.array([float(directory[4:]) for directory in sorted_dirs])[::-1]
...
for directory in [name for name in sorted(os.listdir(load_dir_improved))]:
    area = float(directory[4:])
    areas.append(area)
    fid_im = np.load(load_dir_improved + directory + '/Fidelity.npy')
    opt_par = np.load(load_dir_improved + directory + '/Opt_par.npy')

    system = Optimizer(num_of_atoms=N)
    system.chain()
    system.rydberg_3D_array_vdw(block=B, decay_r=0, power=6, flip_b=True)
    T = area / Omega_0 / 0.75270854
    ...
    system.double_chirped_linear(t=T,
                                 ...
                                 delta=Delta_0,
                                 dev=T / 2 / 2.5, power=8)
    system.configure(input_states =[],
                     target_states=[],
                     initial_guess  =opt_par,
                     opt_pulse_basis='chopped_sines',
                     factors=np.array([1, 2, 1]),
                     mute_mask   =[0, 1],
                     arrangements=None)
    system.update_pulses()
    if area == 31.7813:
...
system = QuantumSimulator(num_of_atoms=N-2)
for ii in range(N-2):
    number_op1 += system.__expand__(matrix=Proj, from_subspace=ii)
system = QuantumSimulator(num_of_atoms=N - 1)
for ii in range(N-1):
    number_op2 += system.__expand__(matrix=Proj, from_subspace=ii)
system = QuantumSimulator(num_of_atoms=N)
for ii in range(N):
    number_op3 += system.__expand__(matrix=Proj, from_subspace=ii)

load_dir = current_dir + '/DATA_N5_randomized/B40_Omega8_Delta16/variance0.01'
# load_dir3 = current_dir + '/DATA_N5_randomized/B40_Omega8_Delta16/variance0.01'

times = np.load(load_dir + '/times.npy')

# print(number_op1.shape)


decay_00 = []
for ii, phi_00 in enumerate(np.load(load_dir + '/histories_00.npy')):
...
decay_01 = []
for ii, phi_01 in enumerate(np.load(load_dir + '/histories_01.npy')):
...
decay_11 = []
for ii, phi_11 in enumerate(np.load(load_dir + '/histories_11.npy')):
...
decay_errors = np.exp(-Gamma * 1/4 * (decay_00 + decay_01 + decay_11))
...
inset.xaxis.set_minor_locator(MultipleLocator(0.05/3))


# ax1.scatter(two_tau3/2, 1-fids3, s=5, lw=0.6, marker='*', facecolors='white', edgecolors='darkblue', alpha=1, zorder=4)



ax1.plot(two_tau/2, 1-fids3[::-1], color='darkblue', lw=0.6, zorder=1)
ax1.scatter(two_tau/2, 1-fids3[::-1], s=5, lw=0.6, facecolors='white', edgecolors='darkblue', alpha=1, zorder=2)
ax1.plot(two_tau/2, 1-fid_r_ave, color='darkred', lw=0.6, zorder=1)
ax1.scatter(two_tau/2, 1-fid_r_ave, s=5, lw=0.6, marker='^', facecolors='white', edgecolors='darkred', alpha=1, zorder=2)

inset.scatter(two_tau/2, 1-fid_r_ave + 1 - decay_errors, s=0.4, lw=0.6, marker='^', color='darkred', alpha=1, zorder=3)
inset.scatter(two_tau/2, 1-fid + 1 - decay_errors, s=0.4, lw=0.6, color='darkblue', alpha=1, zorder=3)
inset.set_ylim(6e-4, 2e-1)


ax1.fill_between(two_tau/2, 1-fid_r_ave, 1-fid_r_ave+fid_errors, color='darkred', alpha=0.2)



ax1_up.scatter(two_theta/2, np.zeros_like(two_tau))
ax1_up.vlines([31.7813/2, 33.8622/2], ymin=1e-9, ymax=100, lw=0.3, linestyles='--', color='k')

print(31.7813/2 / Omega_0 / 0.75270854)
print(33.8622/2 / Omega_0 / 0.75270854)
print(36.6998/2 / Omega_0 / 0.75270854, '\n\n\n\n')
